function storePlayers1() {                                                  //check selection, generate (yds, td, etc) record object, store RECORD obj
            var p = document.getElementsByClassName("team1");
            var pLength;
            var qbCount = 0;
            var rbCount = 0;
            var rxCount = 0;
            var totalCount;
            var checkBoxes = [];
            sessionStorage.setItem("t1Flag", "0");

            function removeTeam1() {
                localStorage.removeItem('qb0');
                localStorage.removeItem('rb1');
                localStorage.removeItem('rb2');
                localStorage.removeItem('wr3');
                localStorage.removeItem('wr4');
            }
            ...
            if (qbCount > 1 || rbCount > 2 || rxCount > 2) {
                ...
                alert("All OK");
                points(p, checkBoxes);
                sessionStorage.setItem("t1Flag", "1");
            }
        }